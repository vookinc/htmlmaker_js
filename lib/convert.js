...
Convert.prototype.convertDocx = function (stylesJson, docx, docxStyles) {
	...
	docx.value.children.forEach(function (para, index, paras) {
      ...
      var prevPara = (index > 0) ? paras[index - 1]: null;
		  convert.convertDocxParagraph(para, styles, docxStyles, prevPara);
...
Convert.prototype.convertDocxParagraph = function (para, styles, docxStyles, prevPara) {
	...
	convert.behaviorFunctions = {
      ...
      console.log(styles);
      console.log(styles.tagForRef);
      console.log(styles.docxStyles['@default']);
			var parent = behavior["parent"];
      console.log(parent);
      var parentTag = styles.tagForRef(parent);
      console.log(parentTag);
			var parentStyle = parentTag && parentTag.dataTypeForDocxStyle(para.styleId);
	...
	function processHtmlEl(tag, tagStyle) {
		var behavior = tagStyle && determineBehavior(tagStyle["behavior"], para, prevPara);
		if (behavior && behavior["name"] == "aggregate") {
	...
	function createElement(tagName, tagStyle) {
		...
		if (tagStyle && tagStyle["data-type"] && tagStyle["data-type"] != "@default") {
			htmlEl.attr("data-type", tagStyle["data-type"]);
		}
		if (tagStyle && tagStyle["class"].length) {
			htmlEl.addClass(tagStyle["class"].join(" "));
		}
		tagStyle && tagStyle["attributes"].forEach(function (attribute) {
